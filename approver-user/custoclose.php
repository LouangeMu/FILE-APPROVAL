...
?>
    <tr>
        ...
        $findasseng=$pdo_conn->prepare("SELECT * FROM tbl_task_assign where p_id='".$row['p_code']."' and ass_stat=1");
        $findasseng->execute();
        $rowfi=$findasseng->fetch();

         $engid=$rowfi['ass_eng'];
        
        $findowner=$pdo_conn->prepare("SELECT * FROM users where u_id='".$engid."'");
      $findowner->execute();
